...
const Account: React.FC = () => {
  ...
  const checkAuthStatus = async () => {
    ...
    if (token) {
      try {
        const response = await fetch('https://inventory-system-server-jh79.onrender.com/api/auth/verify-token', {
          method: 'POST',
          headers: {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://inventory-system-server-jh79.onrender.com/api/auth/login', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://inventory-system-server-jh79.onrender.com/api/auth/create-account', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleGetSecurityQuestions = async (username: string) => {
    ...
    try {
      const response = await fetch(`https://inventory-system-server-jh79.onrender.com/api/auth/security-questions/${username}`);
      const data = await response.json();
          ...
          <TabPanel value={tabValue} index={0}>
            <Box textAlign="center" mb={3}>
              ...
              <Typography variant="body1" color="text.secondary">
                You are successfully logged in as the pharmacy owner.
              </Typography>
            </Box>
        ...
        <TabPanel value={tabValue} index={1}>
          <Box component="form" onSubmit={handleCreateAccount}>
            ...
            <TextField
              ...
              onChange={(e) => setCreateAccountData({ ...createAccountData, securityAnswer1: e.target.value })}
              margin="normal"
              required
              size={isMobile ? "small" : "medium"}
            />
            ...
            <TextField
              ...
              onChange={(e) => setCreateAccountData({ ...createAccountData, securityAnswer2: e.target.value })}
              margin="normal"
              required
              size={isMobile ? "small" : "medium"}
            />
            ...
            {securityQuestions && (
              <>
                <TextField
                  ...
                  onChange={(e) => setResetPasswordData({ ...resetPasswordData, securityAnswer1: e.target.value })}
                  margin="normal"
                  required
                  size={isMobile ? "small" : "medium"}
                />
                <TextField
                  ...
                  onChange={(e) => setResetPasswordData({ ...resetPasswordData, securityAnswer2: e.target.value })}
                  margin="normal"
                  required
                  size={isMobile ? "small" : "medium"}
                />
                <TextField
                  ...
                  onChange={(e) => setResetPasswordData({ ...resetPasswordData, recoveryCode: e.target.value })}
                  margin="normal"
                  required
